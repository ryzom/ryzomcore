 ...
 * Ryzom Recover
 *
 * This application loads/restores minimal application set to connect
 * to Ryzom and play/apply patches.
 *
 * Startup URIs to recover files are hardcoded (see const char* BaseURI[]).
 * First it connects to provided URIs and tries to download a file called
 * 'dir.gz' (see variable DirFilename).
 *
 * file 'dir.gz' has possibly the following forms:
 *
 * ----8<------------8<------------8<--------
 * RECOV
 * client.cfg.gz|client.cfg.gz
 * client_default.cfg.gz|client_default.cfg.gz
 * launch.bat.gz|launch.bat.gz
 * nel_launcher.cfg.gz|nel_launcher.cfg.gz
 * nel_launcher.exe.gz|nel_launcher.exe.gz
 * VERSION.gz|VERSION.gz
 * ----8<------------8<------------8<--------
 *
 * or for the newest version (which provided newer and more up to date URIs to
 * download files from):
 *
 * ----8<------------8<------------8<--------
 * RECOV2
 * ftp://anonymous:password@ftprecover.ryzom.org/recover4/
 * http://ryzomrecover.dyndns.org/recover4/
 * http://recover.ryzom.com/recover4
 * FILELIST
 * client.cfg.gz|client.cfg.gz
 * client_default.cfg.gz|client_default.cfg.gz
 * launch.bat.gz|launch.bat.gz
 ...
 * ----8<------------8<------------8<--------
 *
 * Listed files are then downloaded from base URIs (or preferably from extended URIs
 * if RECOV2 specified). It attempts to download from first URI to last until it found one.
 * If all fail, it loops until success or timeout (see DownloadTimeout). When all servers
 * are not reachable, it wait randomly some seconds (see MinFailureWait and MaxFailureWait)
 *
 * ---------------------------------------------------------- */
...
string DirFilename = "dir.gz";

string NameOfThisApplication;

// These paths are for the open beta
/*const char*BaseURI[] = {	// don't forget the final /
	//"ftp://login:password@server.domain.com/",		// ftp supported in libcurl
	"http://213.208.119.189/recover/",
	"http://213.208.119.190/recover/",
	"http://ryzomrecover.dyndns.org/recover/",
};*/

// These paths are for the *gold* version
const char*BaseURI[] = {	// don't forget the final /
	"http://213.208.119.221/recover4/",	// rspatch
	"http://213.208.119.189/recover4/",	// rsweb
	"http://recover.ryzom.com/recover4/",	// dns link
	"http://recover.ryzom.com:43434/recover4/",	// dns link + other port
	"ftp://anonymous:recover@recover.ryzom.com/recover4/",	// ftp dns link
	"ftp://anonymous:recover@recover.ryzom.com:41414/recover4/",	// ftp dns link + other port
};

// These paths are for the test version
/*const char*BaseURI[] = {	// don't forget the final /
	"http://213.208.119.221/test4/",	// rspatch
	"http://213.208.119.189/test4/",	// rsweb
	"http://recover.ryzom.com/test4/",	// dns link
};*
...
void userFriendlyError( const string& explanation, bool hasDirectionsInExplanation, TErrorStatus errorStatus, const string& techReason, const string& later="" )
{
	...
	do
	{
		...
		clicked = ::MessageBox( NULL, text.c_str(), "Ryzom Recover Application: Error", mbButtons | MB_ICONINFORMATION | MB_SETFOREGROUND );
		switch ( clicked )
		{
		case IDRETRY:
...
int main(int argc, char **argv)
{
	/*printf("\n");
	printf("  **************************************************************************\n");
	printf("  ** THIS IS A TEST THIS IS A TEST THIS IS A TEST.                   v1.1 **\n");
	printf("  ** This might take several minutes depending on your connection.        **\n");
	printf("  ** Please be patient. This window will close when the test     is over. **\n");
	printf("  **************************************************************************\n");
	printf("\n");*/
	printf("\n");
	printf("  **************************************************************************\n");
	printf("  ** We are now patching The Saga of Ryzom on your computer.         v1.1 **\n");
	printf("  ** This might take several minutes depending on your connection.        **\n");
	...
	setDefaultEmailParams ("gw.nevrax.com", "ryzom_recover@recover.com", "lecroart@nevrax.com");
	...
	do
	{
		...
		{
			userFriendlyError( Explain1Internal, false, ESUnrecoverable, "Catched '...'" );
		}
	}
	while ( mustRestart );

	printf( "The Saga of Ryzom was successfully patched by Ryzom Recover.\nNow you can run the game.\n" );
