...
// The sheet libraries expect their host application to define these seam
// symbols (see ryzom/server/src/sheets). None of the code paths that use
// them are actually exercised by this tool (we call loadForm()/loadForm2()
// directly instead of going through CSheets::init()/loadSheetSet()), but
// they still need to exist for the linker since the whole egs_sheets.cpp
// object is pulled in.
...
// Same for the ai sheets library: creature fight script components route
// through this seam; dumping only cares about the serialized strings.
