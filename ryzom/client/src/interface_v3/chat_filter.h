...
public:
	...
	/** Display a msg in the chat. The msg will be forwarded to all the listening windows
	  ...
	  */
	void displayMessage(const std::string &msg, NLMISC::CRGBA col, uint numBlinks = 0, bool *windowVisible = nullptr);
	/** The same as displayMessage, but with sender name, so that the msg will be displayed in attached people lists as well
	  */
	void displayTellMessage(/*TDataSetIndex &senderIndex, */const std::string &msg, const std::string &sender, NLMISC::CRGBA col, uint numBlinks = 0, bool *windowVisible = nullptr);
