...
function r2:group(src, target)
	...
	elseif target:isInDefaultFeature() then
		...
		local uc_groupName = ""
		uc_groupName = r2.PaletteIdToGroupTranslation[src.Base].. " " .. i18n.get("uiR2EDNameGroup")
		local groupName = r2:genInstanceName(uc_groupName)	
...
function r2:copy()
   ...
   displaySystemInfo((selection:getDisplayName() .. i18n.get("uiR2EDSelectionCopied")), "BC")
end

------------------------------------------------------------------------------------
-- called by C++ when copy has been pressed (key or ui button)
function r2:paste()	
	r2.requestNewAction((i18n.get("uiR2EDPasteAction") .. r2.ClipBoardDisplayName))
...
function r2:popMainMenu()	
	...
	menu:find("preferences_key").t.hardtext = buildKeyName("show_hide", "game_config")
	menu:find("keys_key").t.hardtext = buildKeyName("show_hide", "keys")
	--menu:find("debug_console_key").t.hardtext = buildKeyName("show_hide", "debug_info")
	menu:find("chat_window_key").t.hardtext = buildKeyName("show_hide", "main_chat")		
	menu:find("quit_key").t.hardtext = buildKeyName("quit_ryzom_now", "")   
	menu:find("mail_box_key").t.hardtext = buildKeyName("show_hide", "mailbox")
	menu:find("guild_forum_key").t.hardtext = buildKeyName("show_hide", "guild_forum")    
	-- fill name of key for each menu entry
	if r2.Mode == "Edit" then
		menu:find("go_test_key").t.hardtext = buildKeyName("r2ed_try_go_test", "")
		menu:find("palette_key").t.hardtext = buildKeyName("show_hide", "r2ed_palette")
		menu:find("scenario_key").t.hardtext = buildKeyName("show_hide", "r2ed_scenario")
		menu:find("cust_bbox").active = (config.R2EDExtendedDebug == 1)		
	else
		menu:find("stop_test_key").t.hardtext = buildKeyName("r2ed_stop_test", "")
	...
	menu:find("map_key").t.hardtext = buildKeyName("show_hide", "map")
...
function r2:onModeChanged()
	...
	if r2.Mode == "Edit" and r2.LastTranslationErrorMsg ~= nil then		
		...
		local ucStringMsg = "Translation Error"
		--ucStringMsg = r2.LastTranslationErrorMsg		
...
function r2:noMoreRoomLeftInPrimitveMsg()
	...
	local msg = (i18n.get("uiR2EDNoMoreRoomInPrimStart") .. keyName .. i18n.get("uiR2EDNoMoreRoomInPrimEnd"))
...
function r2:onUndo(actionUCName)
	debugInfo("*undo*")
	displaySystemInfo((i18n.get("uiR2EDUndoing") .. actionUCName), "BC")
...
function r2:onRedo(actionUCName)
	debugInfo("*redo*")
	displaySystemInfo((i18n.get("uiR2EDRedoing") .. actionUCName), "BC")
	r2.ToolUI:updateUndoRedo()
end
