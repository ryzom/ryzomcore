...
function r2:buildActivityTitle(activityUI, erase)
	...
	assert(title)
	title.hardtext= part1..part2..part3
end
...
function r2:updateActivityEditor()
	...
	if instanceActivity then
		...
		if index~= nil then
			activityName.hardtext = tostring(i18n.get("uiR2EdActivity")).." "..index.." : "
		else
			activityName.hardtext = tostring(i18n.get("uiR2EdActivity")).." : "
		end
		
		-- activity type
		local activityText = r2.activityTypeMenu[instanceActivity.Activity]
		if activityText then
			activityButtonText.hardtext = activityText
		end
		if instanceActivity.ActivityZoneId ~= "" then
			local place = r2:getInstanceFromId(instanceActivity.ActivityZoneId)
			assert(place)

			activityButtonText.hardtext = activityButtonText.hardtext .. " " .. place.Name
		end
		...
		if timeLimit == "Few Sec" then
			...
			if timeLimitValue ~= nil then
				...
				assert(hoursMenu)
				hoursMenu.hardtext = tostring(hoursNb)

				local minutesMenu = certainTime:find("minutes"):find("text")
				assert(minutesMenu)
				minutesMenu.hardtext = tostring(minNb)

				local secondsMenu = certainTime:find("seconds"):find("text")
				assert(secondsMenu)
				secondsMenu.hardtext = tostring(secNb)
		...
		if sequenceChat ~= nil then
			--chatMenuButton.hardtext = sequenceChat.Name
			chatMenuButton.hardtext = r2:getSequenceName(sequenceChat)
			...
			chatMenuButton.hardtext = i18n.get("uiR2EdNoElt")
		...
		activityName.hardtext = name
		
		activityButtonText.hardtext = i18n.get("uiR2EdStandStill")
...
function r2:updateChatEditor()
	...
	if instanceChat then
		...
		if index~= nil then
			chatName.hardtext = tostring(i18n.get("uiR2EdChat")).." "..index.." : "
		else
			chatName.hardtext = tostring(i18n.get("uiR2EdChat")).." : "
		end
		...
		minutesText.hardtext = tostring(minNb)
		secondsText.hardtext = tostring(secNb)
			
		-- who
		local whoInst = r2:getInstanceFromId(tostring(instanceChat.Actions[0].Who))
		whoMenuText.hardtext = whoInst.Name
		...
		if toWhoInst then
			toWhoMenuText.hardtext = toWhoInst.Name
		else
			toWhoMenuText.hardtext = tostring(i18n.get("uiR2EdNobody"))
		end

		-- emote
		local emoteName = r2.fromEmoteIdToName[instanceChat.Actions[0].Emote]
		if emoteName then
			emoteButtonText.hardtext = emoteName
		else
			emoteButtonText.hardtext = tostring(i18n.get("uiR2EdNoElt"))
		end

	else

		local name = tostring(i18n.get("uiR2EdChat")).." : "
		chatName.hardtext = name

		minutesText.hardtext = tostring(0)
		secondsText.hardtext = tostring(0)

		whoMenuText.hardtext = ""
...
function r2:openSequenceEditor(uiName, editorName)
	...
	editName.input_string = buttonTab.hardtext
end
...
function r2:updateSequencesButtonBar(index, sequenceName)
	...
	if activeLogicEntity then
		if sequenceName==nil and (index >=0) and (index < activeLogicEntity:getBehavior().Activities.Size) then
			...
			assert(activitySequence)
			--sequencesButton.hardtext = activitySequence.Name
			sequencesButton.hardtext = r2:getSequenceName(activitySequence)
		elseif sequenceName~= nil then
			sequencesButton.hardtext = sequenceName
		else
			sequencesButton.hardtext = i18n.get("uiR2EDSequences")
		end
	end
end
...
function r2:updateMiniActivityView(index)
	...
	if sequence~=nil then
		...
		if sequence.Components.Size == 0 then
			...
			noActivityLabel.hardtext = tostring(i18n.get("uiR2EdNoActivity"))
		else
		...
		for i=0, sequence.Components.Size-1 do
			...
			if activityInst then
				...
				assert(activityText)
				activityText.hardtext = activityInst.Activity
			end
		end
		startCount = sequence.Components.Size 
	else
		noActivityLabel.active = true
		noActivityLabel.hardtext = tostring(i18n.get("uiR2EdNoSequence"))
	end
...
function r2:chooseOrOpenSelectedChatSequence()
	...
	if chatSequenceId ~= "" then
		...
		assert(chatSequence)
		--rootMenu:addLine(tostring(i18n.get("uiR2EDChatSequence")).." " .. chatSequence.Name .." : ", "lua", "", "Title")
		rootMenu:addLine(tostring(i18n.get("uiR2EDChatSequence")).." " .. r2:getSequenceName(chatSequence) .." : ", "lua", "", "Title")
	else
		rootMenu:addLine(tostring(i18n.get("uiR2EDChatSequence")).." : ", "lua", "", "Title")
	end
	...
	-- "Open chat sequence"
	if chatSequenceId ~= "" then
		--rootMenu:addLine(tostring(i18n.get("uiR2EDOpen")).." "..chatSequence.Name, "lua", "r2:openMiniActivityChatSequence("..tostring(miniActivityNb)..")", "Open")
		rootMenu:addLine(tostring(i18n.get("uiR2EDOpen")).." "..r2:getSequenceName(chatSequence), "lua", "r2:openMiniActivityChatSequence("..tostring(miniActivityNb)..")", "Open")
		newLine = newLine + 1
	end
	...
	-- "new chat sequence"
	rootMenu:addLine(tostring(i18n.get("uiR2EdNewChat")).."...", "lua", "r2:newChatsSequenceAndSelect("..tostring(miniActivityNb)..")", "None")
