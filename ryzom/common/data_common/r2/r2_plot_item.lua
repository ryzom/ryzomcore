...
function plotItem.getDisplayName(this)
   r2.ScratchUCStr = this.Name
   return (plotItemNamePrefix .. r2.ScratchUCStr)
end
...
local itemUCName = "" -- keep a ucstring to avoid creation of string on the fly
local itemUCDesc = "" -- keep a ucstring to avoid creation of string on the fly
local itemUCComment = "" -- keep a ucstring to avoid creation of string on the fly
...
function r2.PlotItemDisplayerCommon:updateNameAndAvailability(instance, sheetId)
	...
	elseif plotItemSheetNbRef[sheetId] > 1 then
		...
		itemUCName = instance.Name
		itemUCDesc = instance.Desc
		itemUCComment = instance.Comment
...
function r2.PlotItemDisplayerCommon:updateAll()
	...
	if index <= maxNumPlotItems - 1 then
		...
		slot.active=true
		slot.t.hardtext = i18n.get("uiR2EDCreateNewItem")
