...
public:
	...
	virtual void sendUniverseChatShared(const ucstring &senderName, uint32 homeSessionId, const CChatMessage &message) = 0;
	virtual void sendUnifiedDynChat(const NLMISC::CEntityId &dynCharId, const ucstring &senderName, const ucstring &text) = 0;
	virtual void sendUnifiedDynChatShared(const NLMISC::CEntityId &dynCharId, const ucstring &senderName, const CChatMessage &message) = 0;
};
