...
// EGS-side definitions for the seams of the egs sheets library. The sheet
// classes compile once into ryzom_egssheets; the few methods and globals
// that depend on EGS runtime state (craft parameters, quartering variables,
// the service write directory, the shared random generator) live here so the
// library object files carry no undefined references to EGS internals. The
// sheets_packer_shard tool provides its own writeDirectory and never calls
