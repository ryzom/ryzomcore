...
namespace PIPELINE
{
	...
	void CModulePipelineMasterSkel::slaveFinishedBuildTask_skel(NLNET::IModuleProxy *sender, const NLNET::CMessage &__message)
	{
		...
		slaveFinishedBuildTask(sender, errorLevel);
	}
	...
	void CModulePipelineMasterSkel::setAvailablePlugins_skel(NLNET::IModuleProxy *sender, const NLNET::CMessage &__message)
	{
		...
		// 
	void CModulePipelineMasterProxy::slaveFinishedBuildTask(NLNET::IModule *sender, uint8 errorLevel)
	{
		if (_LocalModuleSkel && _LocalModule->isImmediateDispatchingSupported())
		{
			// immediate local synchronous dispatching
			_LocalModuleSkel->slaveFinishedBuildTask(_ModuleProxy->getModuleGateway()->getPluggedModuleProxy(sender), errorLevel);
		}
		else
		{
			...
			buildMessageFor_slaveFinishedBuildTask(__message, errorLevel);
	...
	const NLNET::CMessage &CModulePipelineMasterProxy::buildMessageFor_slaveFinishedBuildTask(NLNET::CMessage &__message, uint8 errorLevel)
	{
