...
char * ConnectionStateCStr [9] = { "NotInitialised", "NotConnected", "Authenticate", "Login", "Synchronize", "Connected", "Probe", "Stalled", "Disconnect" };
...
void	CNetworkConnection::init(const string &cookie, const string &addr)
{
	...
	// Specific for Client Drone
	CCDBNodeBranch *root = new CCDBNodeBranch("Root");
	CCDBNodeBranch *entities = new CCDBNodeBranch("Entities");
	for (uint slot=0; slot!=256; ++slot)
	{
		CCDBNodeBranch *b = new CCDBNodeBranch(toString("%Eu", slot));

		for (uint p=0; p!=CLFECOMMON::NB_VISUAL_PROPERTIES; ++p)
		{
			CCDBNodeLeaf *l = new CCDBNodeLeaf(toString("%Pu", p));
			b->attachChild(l, *l->getName());
		}

		entities->attachChild(b, *b->getName());
	}
	root->attachChild(entities, *entities->getName());
	setDataBase(root);
}
...
//ace faut faire la nouveau login client 	result = CLoginClient::connectToShard (_FrontendAddress, _Connection);
	...
	{
		result = toString ("FES refused the connection (%s)", e.what());
		return false;
	}
		...
		if (buildStream(msgin) && decodeHeader(msgin))
		{
			if (_SystemMode)
			{
				...
				case SYSTEM_STALLED_CODE:
					...
					nlwarning("CNET[%p]: login->stalled", this);
...
#endif
	uint	dt = uint(_UpdateTime - _CurrentClientTime);
...
//					nlinfo( "Cannot disassociate slot %hu: sheet not received yet", (uint16)slot );
			...
			if ( currentNode->a()->BranchHasPayload )
			{
				...
				*/
				...
				thechange.PositionInfo.PredictedInterval = 0; //predictedInterval;
...
//			nlinfo ("Associating S%hu", (uint16)slot );
			...
			thechange.NewEntityInfo = (TClientDataSetIndex)(ac->getValue() >> 32);
			_Changes.push_back( thechange );
			...
			if (_DataBase != NULL)
			{
				...
				if ( nodeRoot )
				{
					...
					nlassert(node != NULL);
					node->setValue64(ac->getValue() & 0xFFFFFFFF);
					if ( LoggingMode )
					{
						nlinfo( "CLIENT: recvd property %hu (%s) for slot %hu, date %u", (uint16)propIndex, getPropText(propIndex), (uint16)slot, TVPNodeClient::SlotContext.Timestamp );
					}
...
bool	CNetworkConnection::stateConnected()
{
	...
	while ((_ConnectionState != Disconnect) && _Connection.dataAvailable())// && _TotalMessages<5)
	{
		_DecodedHeader = false;
		CBitMemStream	msgin (true);
		...
		if (buildStream(msgin) && decodeHeader(msgin, false))
		{
			if (_SystemMode)
			{
				...
				case SYSTEM_SYNC_CODE:
					...
					reset();
					_ConnectionState = Synchronize;
					nlwarning("CNET[%p]: quit->synchronize", this);
					receiveSystemSync(msgin);
					return true;
					break;
/*
...
void		CNetworkConnection::updateSmoothServerTick()
{
	...
	else
	{
		...
		// Update the Smooth
		sint32 msPerTick = getMsPerTick();
		if (msPerTick == 0)
		  msPerTick = 1; // workaround for client_drone under Linux
		_CurrentSmoothServerTick+= ((deltaT*SMOOTH_SERVER_TICK_PER_TICK*factor)/msPerTick) >> 16;
	}
}
 ...
 *
 */
sint64 CNetworkConnection::getVisualProp(CLFECOMMON::TCLEntityId slot, CLFECOMMON::TProperty prop) const
{
	CCDBNodeBranch	*nodeRoot = dynamic_cast<CCDBNodeBranch*>(_DataBase->getNode((uint16)0));
	if (!nodeRoot)
		return ~0;

	CCDBNodeLeaf	*node  = dynamic_cast<CCDBNodeLeaf*>(nodeRoot->getNode(slot)->getNode(prop));
	if (!node)
		return ~0;
