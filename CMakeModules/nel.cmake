...
IF(HUNTER_ENABLED)
  ...
  IF(WIN32 AND WITH_DRIVER_DIRECT3D)
    TARGET_LINK_LIBRARIES(${name} PRIVATE nel_drv_direct3d${drv_suffix})
  ENDIF()
  IF(WITH_DRIVER_OPENGL)
    TARGET_LINK_LIBRARIES(${name} PRIVATE nel_drv_opengl${drv_suffix})
  ENDIF()
  IF(WITH_DRIVER_OPENGL3)
    TARGET_LINK_LIBRARIES(${name} PRIVATE nel_drv_opengl3${drv_suffix})
  ENDIF()
  IF(WITH_DRIVER_OPENGLES)
    TARGET_LINK_LIBRARIES(${name} PRIVATE nel_drv_opengles${drv_suffix})
  ENDIF()
  IF(WITH_DRIVER_OPENGLES3)
    TARGET_LINK_LIBRARIES(${name} PRIVATE nel_drv_opengles3${drv_suffix})
  ENDIF()
ELSE()
  IF(WITH_STATIC_DRIVERS)
    IF(WIN32)
      IF(WITH_DRIVER_DIRECT3D)
        TARGET_LINK_LIBRARIES(${name} PRIVATE nel_drv_direct3d_win)
      ENDIF()
    ENDIF()

    IF(WITH_DRIVER_OPENGL)
      IF(WIN32)
        TARGET_LINK_LIBRARIES(${name} PRIVATE nel_drv_opengl_win)
      ELSE()
        TARGET_LINK_LIBRARIES(${name} PRIVATE nel_drv_opengl)
      ENDIF()
    ENDIF()
	
    IF(WITH_DRIVER_OPENGL3)
      IF(WIN32)
        TARGET_LINK_LIBRARIES(${name} PRIVATE nel_drv_opengl3_win)
      ELSE(WIN32)
        TARGET_LINK_LIBRARIES(${name} PRIVATE nel_drv_opengl3)
      ENDIF()
    ENDIF()

    IF(WITH_DRIVER_OPENGLES)
      IF(WIN32)
        TARGET_LINK_LIBRARIES(${name} PRIVATE nel_drv_opengles_win)
      ELSE()
        TARGET_LINK_LIBRARIES(${name} PRIVATE nel_drv_opengles)
      ENDIF()
    ENDIF()

    IF(WITH_DRIVER_OPENGLES3)
      IF(WIN32)
        TARGET_LINK_LIBRARIES(${name} PRIVATE nel_drv_opengles3_win)
      ELSE()
        TARGET_LINK_LIBRARIES(${name} PRIVATE nel_drv_opengles3)
      ENDIF()
    ENDIF()
  ENDIF()
ENDIF()#hunter
