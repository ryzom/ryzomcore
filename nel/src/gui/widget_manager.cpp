...
namespace NLGUI
{
	...
	void CWidgetManager::reset()
	{
		...
		_ContextHelpContainerColorDB = NULL;
	...
	// ------------------------------------------------------------------------------------------------
	void CWidgetManager::updateContextHelpColors()
	{
		// TODO: should only update when dbProp changes and not on every frame
		if (!_ContextHelpContentAlphaDB)
		{
			_ContextHelpContentAlphaDB = CDBManager::getInstance()->getDbProp("UI:SAVE:CONTEXT_HELP_CONTENT_ALPHA");
			_ContextHelpContainerColorDB = CDBManager::getInstance()->getDbProp("UI:SAVE:CONTEXT_HELP_CONTAINER_COLOR");
		...
		ip.setNodePtr(_ContextHelpContainerColorDB);
		_ContextHelpContainerColor = ip.getRGBA();
		_ContextHelpContainerColor.A = (uint8) _ContextHelpContainerAlphaDB->getValue32();
	}
	...
	void CWidgetManager::drawContextHelp ()
	{
		...
		if(_CurCtrlContextHelp)
		{
			...
			if(groupContextHelp)
			{
				updateContextHelpColors();

				// setup new global color for tooltip
				NLMISC::CRGBA oldGlobalColorForContent = getGlobalColorForContent();
				NLMISC::CRGBA newGlobalColorForContent = oldGlobalColorForContent;
				newGlobalColorForContent.A = _ContextHelpContentAlpha;
				setGlobalColorForContent(newGlobalColorForContent);

				// container alpha
				CViewBase *pVB = groupContextHelp->getView("midcolor");
				if (pVB)
				{
					CViewBitmap *bg = dynamic_cast<CViewBitmap*>(pVB);
					if (bg)
					{
						bg->setColor(_ContextHelpContainerColor);
					}
				}
				...
				setGlobalColorForContent(oldGlobalColorForContent);
			}
		}
...
//		if ((eventDesc.getKeyEventType() == CEventDescriptorKey::keydown) && !_ModalStack.empty() && !eventDesc.getKeyAlt() && !eventDesc.getKeyCtrl() && !eventDesc.getKeyShift())
	...
	CWidgetManager::CWidgetManager()
	{
		...
		_ContextHelpContainerColor = NLMISC::CRGBA(255,255,159,255);
