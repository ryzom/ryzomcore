...
// Pipeline-stage UBO body for FXAA VP.
// modelViewProjection comes from NlModel UBO (UsesObjectUBO).
const char *a_glsl300esv =
	"#version 300 es\n"
	...
	"  texCoord0 = vec4(vtexCoord0.xy, 0.0, 0.0);\n"
	"}\n";
...
// Hand-resolved for: FXAA_PC, Quality Preset 12 (5 steps), Green-as-Luma, No Early Exit
//
// Pipeline-stage UBO body for FXAA PP.
// FXAA params come from NlFXAA user UBO (UBBindingPixelProgram).
...
// The FXAA algorithm body is shared between UBO and SSO paths
const char *a_fxaaBody =
	"void main()\n"
	"{\n"
	"  vec2 posM = texCoord0.xy;\n"
	"  vec4 rgbyM = textureLod(sampler0, posM, 0.0);\n"
	"  float lumaM = rgbyM.y;\n"
	"\n"
	"  float lumaS = textureLodOffset(sampler0, posM, 0.0, ivec2( 0, 1)).y;\n"
	"  float lumaE = textureLodOffset(sampler0, posM, 0.0, ivec2( 1, 0)).y;\n"
	"  float lumaN = textureLodOffset(sampler0, posM, 0.0, ivec2( 0,-1)).y;\n"
	"  float lumaW = textureLodOffset(sampler0, posM, 0.0, ivec2(-1, 0)).y;\n"
	"\n"
	...
	"\n"
	"  if (range < rangeMaxClamped) { fragColor = rgbyM; return; }\n"
	"\n"
	"  float lumaNW = textureLodOffset(sampler0, posM, 0.0, ivec2(-1,-1)).y;\n"
	"  float lumaSE = textureLodOffset(sampler0, posM, 0.0, ivec2( 1, 1)).y;\n"
	"  float lumaNE = textureLodOffset(sampler0, posM, 0.0, ivec2( 1,-1)).y;\n"
	"  float lumaSW = textureLodOffset(sampler0, posM, 0.0, ivec2(-1, 1)).y;\n"
	"\n"
	...
	"  float lumaEndN = textureLod(sampler0, posN, 0.0).y;\n"
	"  float subpixE = subpixC * subpixC;\n"
	"  float lumaEndP = textureLod(sampler0, posP, 0.0).y;\n"
	"\n"
	...
	"\n"
	"  if (doneNP) {\n"
	"    if (!doneN) lumaEndN = textureLod(sampler0, posN, 0.0).y;\n"
	"    if (!doneP) lumaEndP = textureLod(sampler0, posP, 0.0).y;\n"
	...
	"\n"
	"  fragColor = vec4(textureLod(sampler0, posM, 0.0).xyz, lumaM);\n"
	"}\n";
