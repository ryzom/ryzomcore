...
// ***************************************************************************
void CDriverD3D::enableLightTableMode(bool enable)
{
	H_AUTO_D3D(CDriverD3D_enableLightTableMode);
	_LightTableMode = enable;
}

// ***************************************************************************
void CDriverD3D::setLightTableSize(uint count)
{
	H_AUTO_D3D(CDriverD3D_setLightTableSize);
	_LightTable.resize(count);
}

// ***************************************************************************
void CDriverD3D::setLightTableEntry(uint index, const CLight &light)
{
	H_AUTO_D3D(CDriverD3D_setLightTableEntry);
	if (index < _LightTable.size())
		_LightTable[index] = light;
}

// ***************************************************************************
void CDriverD3D::setLights(
	...
	NLMISC::CRGBA ambient)
{
	...
	uint count = min(numLights, (uint)MaxLight);
	...
	for (uint i = count; i < _MaxLight; ++i)
	{
		_UserLightEnable[i] = false;
		enableLightInternal(uint8(i), false);
	}
