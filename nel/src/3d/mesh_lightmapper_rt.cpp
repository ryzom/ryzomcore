...
// -----------------------------------------------------------------------------------------------
void CRTWorld::build (vector<SLightBuild> &AllLights, const CVector &trans,
	const CLightmapScene &scene, const set<string> &excludeNames,
	const string &includeName,
	const CMesh::CMeshBuild &includeMB, const CMeshBase::CMeshBaseBuild &includeMBB)
{
	...
	// Get all the world meshes from the scene-graph occluder list (replaces the original's
	// Max scene walk; the selection/accelerator/zone filters ran at export time). The
	// receiver geometry itself rides the same filters, exactly like the original's
	// includeNode set (skipped in the occluder pass when it is also listed there).
	for (i = 0; i < scene.Occluders.size(); ++i)
	{
		const CLightmapSceneMesh &occ = scene.Occluders[i];
		if (occ.NodeName == includeName)
			continue;
		if (excludeNames.find(occ.NodeName) != excludeNames.end())
			continue;
		addMesh(occ.NodeName, occ.MeshBuild, occ.BaseBuild, AllLights);
	}
	addMesh(includeName, includeMB, includeMBB, AllLights);

	// Transform the meshbuilds vertices and normals to have world coordinates
	for( i = 0; i < vMB.size(); ++i )
	{
		CMeshLightmapper::convertToWorldCoordinate (vMB[i], vMBB[i], GlobalTrans);
	}
	...
	for (i = 0; i < AllLights.size(); ++i)
	{
		...
		switch (rLight.Type)
		{
			...
			case SLightBuild::LightPoint:
			{
				...
				{
					if (rLight.setExclusion.find (vNodeName[j]) != rLight.setExclusion.end())
						continue;
					...
					for (k = 0; k < vMB[j]->Faces.size(); ++k)
					{
						...
						CBSphere lightSphere (CVector(0,0,0), rLight.rRadiusMax);
						if (intersectionTriangleSphere (tri, lightSphere))
...
// -----------------------------------------------------------------------------------------------
void CRTWorld::addMesh (const string &name, const CMesh::CMeshBuild &mb,
	const CMeshBase::CMeshBaseBuild &mbb, vector<SLightBuild> &AllLights)
{
	// The original's addNode filters, on scene-graph data: the isMesh/zone/accelerator
	// tests ran at export time; the shadow-casting flag and the light-interaction bbox
	// test run here (bbox from the object-space build, like buildMeshAABBox did).
	if (!mbb.bCastShadows)
		return;
	...
	CAABBox aabbox;
	{
		CMatrix MBMatrix = CMeshLightmapper::getObjectToWorldMatrix (&mb, &mbb);
		aabbox.setCenter (MBMatrix * mb.Vertices[0]);
		for (uint32 j = 1; j < mb.Vertices.size(); ++j)
			aabbox.extend (MBMatrix * mb.Vertices[j]);
	}
	...
	if (bInteract)
	{
		vMB.push_back(new CMesh::CMeshBuild(mb));
		vMBB.push_back(new CMeshBase::CMeshBaseBuild(mbb));
		vNodeName.push_back(name);
	}
}
...
bool CRTWorld::intersectionTriangleSphere( CTriangle &t, CBSphere &s )
{
	...
	if( newRadius > 1.0 )
		newRadius = 1.0;
	newRadius = cosf( newRadius * (float)Pi / 2.0f );
...
/* End of mesh_lightmapper_rt.cpp */
