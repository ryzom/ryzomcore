...
#include "mesh_lightmapper_plane.h"
#include "mesh_lightmapper_rt.h"
...
static CMeshLightmapper::COptions gOptions;
...
// -----------------------------------------------------------------------------------------------
CMatrix CMeshLightmapper::getObjectToWorldMatrix( const CMesh::CMeshBuild *pMB, const CMeshBase::CMeshBaseBuild *pMBB )
{
	CMatrix m1;
...
CAABBox getMeshBBox (CMesh::CMeshBuild& rMB, CMeshBase::CMeshBaseBuild &rMBB, bool bNeedToTransform)
{
	...
	if( bNeedToTransform )
	{
		CMatrix MBMatrix = CMeshLightmapper::getObjectToWorldMatrix (&rMB, &rMBB);
...
// -----------------------------------------------------------------------------------------------
void CMeshLightmapper::convertToWorldCoordinate (CMesh::CMeshBuild *pMB, CMeshBase::CMeshBaseBuild *pMBB, const CVector &translation)
{
	uint32 j, k;
	CMatrix MBMatrix = CMeshLightmapper::getObjectToWorldMatrix (pMB, pMBB);
...
// -----------------------------------------------------------------------------------------------
static void supprLightNoInteractOne( vector<SLightBuild> &vLights, CMesh::CMeshBuild* pMB, CMeshBase::CMeshBaseBuild *pMBB, const string &nodeName )
{
	uint32 i;
	...
	for( i = 0; i < vLights.size(); ++i )
	{
		...
		if( vLights[i].setExclusion.find(nodeName) != vLights[i].setExclusion.end() )
		{
			bInteract = false;
		}
		else
...
bool CMeshLightmapper::calculateLM( CMesh::CMeshBuild *pZeMeshBuild, CMeshBase::CMeshBaseBuild *pZeMeshBaseBuild,
							const CLightmapScene &scene, const CLightmapReceiverGeom &geom,
							const COptions &options)
{
	nldebug("Calculate LM: '%s'", geom.NodeName.c_str());

	const uint firstMaterial = geom.FirstMaterial;
	const bool outputLightmapLog = options.OutputLightmapLog;

	uint32 i, j;

	// **** Retrieve Shape Node properties (from the scene-graph receiver record)
	float rLumelSizeMul = geom.LumelSizeMul;
	// 8Bits LightMap Compression
	bool	lmcEnabled= geom.LmcEnabled;
	enum	{NumLightGroup= 3};
	CRGBA	lmcAmbient[NumLightGroup];
	CRGBA	lmcDiffuse[NumLightGroup];
	for(i=0;i<NumLightGroup;i++)
	{
		lmcAmbient[i]= geom.LmcAmbient[i];
		lmcDiffuse[i]= geom.LmcDiffuse[i];
	}
	...
	CMatrix mtmp = CMeshLightmapper::getObjectToWorldMatrix (pZeMeshBuild, pZeMeshBaseBuild);
	...
	// Get all meshes that are influenced by the lights L. The LOD exclusion sets
	// (the original's addChildLodNode/addParentLodNode Max-scene walks) were resolved
	// at export time into geom.ExcludeOccluders; the receiver itself is included.
	WorldRT.build (AllLights, -vGlobalPos, scene, geom.ExcludeOccluders, geom.NodeName,
		*pZeMeshBuild, *pZeMeshBaseBuild);
	...
	//for( nNode=0; nNode < nNbMesh; ++nNode )
	{
		...
		// Get the name of the source scene project
		const std::string &projectName = scene.ProjectName;

		// Add lightmap information in the lightmap log
		COFile outputLog;
		if (outputLightmapLog)
			createLightmapLog(outputLog, gOptions.sExportLighting.c_str(), projectName.c_str(), geom.NodeName.c_str());
		...
		for (j = 0; j < LightMap.nNbLayerUsed; ++j)
		{
			...
			if (!LightMap.isAllBlack((uint8)j) || lmcLayerAmbient!=CRGBA::Black)
			{
				...
				string sSaveName = geom.NodeName;
				char tmp[32];
				sSaveName += "_";
				...
				sSaveName = CPath::standardizePath (gOptions.sExportLighting, true);
				sSaveName += pLightMap->getFileName();
				// (the original's _AbsolutePath in-Max-viewer mode set the full path as the
				// texture name; the batch export path always keeps the relative name)
				...
				{
					nlwarning("Can't write the file %s : %s", sSaveName.c_str(), e.what());
				}
