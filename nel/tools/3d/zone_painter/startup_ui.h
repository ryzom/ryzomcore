...
struct SStartupSelection
{
	...
	/** Unused; always empty / "1x1". Prefer --place or the scratch board for instances. */
...
// (home + place/rotate/mirror/remove). Instance layout is board-driven (--place / scratch board).
 ...
 *
 * Ecosystem scratch idiom (; every open file — the first-opened included — is
...
/** Dev/test: open the close-confirm modal for a screenshot frame. */
void forceShowCloseConfirmForShot(const std::string &basename = std::string());

/** Dev/test: open the cell-action popup for a screenshot frame. */
void forceShowCellActionForShot(const std::string &basename = std::string());

/** Dev/test: open the instance-action popup (ecosystem scratch) for a screenshot frame. */
void forceShowInstanceActionForShot(const std::string &basename = std::string());

/** Dev/test: open the empty-cell popup (ecosystem scratch) for a screenshot frame. */
void forceShowEmptyCellForShot(const std::string &basename = std::string());

/** Dev/test: open the context-brick action popup (ecosystem scratch) for a screenshot frame. */
