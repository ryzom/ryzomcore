...
class CUTMiscPackFileTest : public testing::Test
{
protected:
	string _WorkingPath;
	string _OldPath;
	...
	void loadFromBnp()
	{
		...
		ASSERT_TRUE(filename == "files.bnp@file1_in_bnp.txt");
		...
		// read the first file content
		{
			...
			ASSERT_TRUE(content1 == "The content of the first file");
		}

		// lookup for the 2nd file
		filename = NLMISC::CPath::lookup("file2_in_bnp.txt", true, true, false);
		ASSERT_TRUE(filename == "files.bnp@file2_in_bnp.txt");

		{
			...
			// check the file content
			ASSERT_TRUE(content2 == "Another content but for the second file");
		}
	}
	void loadFromXmlpack()
	{
		...
		ASSERT_TRUE(filename == NEL_UNIT_BASE "ut_misc_files/xml_files/xml_files.xml_pack@@file1_in_xml_pack.xml");
		...
		// read the first file content
		{
			...
			ASSERT_TRUE(content1 == refText);
		}

		// lookup for the 2nd file
		filename = NLMISC::CPath::lookup("file2_in_xml_pack.xml", true, true, false);
		ASSERT_TRUE(filename == NEL_UNIT_BASE "ut_misc_files/xml_files/xml_files.xml_pack@@file2_in_xml_pack.xml");

		{
			...
			ASSERT_TRUE(content2 == refText);
		}
	}
};

TEST_F(CUTMiscPackFileTest, addBnp)
{
...
TEST_F(CUTMiscPackFileTest, loadFromBnp)
{
	loadFromBnp();
}

TEST_F(CUTMiscPackFileTest, addXmlpack)
{
...
TEST_F(CUTMiscPackFileTest, loadFromXmlpack)
{
	loadFromXmlpack();
}

TEST_F(CUTMiscPackFileTest, compressMemory)
{
	// #ifdef WIN32
	//_CrtCheckMemory();
	// #endif
...
TEST_F(CUTMiscPackFileTest, loadFromBnpCompressed)
{
	// simply recall loadFromBnp
	loadFromBnp();
}

TEST_F(CUTMiscPackFileTest, loadFromXmlpackCompressed)
{
	// simply recall loadFromXmlpack
	loadFromXmlpack();
}

TEST_F(CUTMiscPackFileTest, decompressMemory)
{
	NLMISC::CPath::memoryUncompress();
}

TEST_F(CUTMiscPackFileTest, loadFromBnpUncompressed)
{
	// simply recall loadFromBnp
	loadFromBnp();
}

TEST_F(CUTMiscPackFileTest, loadFromXmlpackUncompressed)
{
	// simply recall loadFromXmlpack
	loadFromXmlpack();
}

TEST_F(CUTMiscPackFileTest, loadXmlpackWithSameName)
{
	...
	ASSERT_TRUE(filename == NEL_UNIT_BASE "ut_misc_files/xml_files/same_subfolder_1/samename/samename.xml_pack@@file1_in_sub_1.xml");
	filename = NLMISC::CPath::lookup("file2_in_sub_1.xml", true, true, false);
	ASSERT_TRUE(filename == NEL_UNIT_BASE "ut_misc_files/xml_files/same_subfolder_1/samename/samename.xml_pack@@file2_in_sub_1.xml");

	// lookup for the files in the second subdirectory
	filename = NLMISC::CPath::lookup("file1_in_sub_2.xml", true, true, false);
	ASSERT_TRUE(filename == NEL_UNIT_BASE "ut_misc_files/xml_files/same_subfolder_2/samename/samename.xml_pack@@file1_in_sub_2.xml");
	filename = NLMISC::CPath::lookup("file2_in_sub_2.xml", true, true, false);
	ASSERT_TRUE(filename == NEL_UNIT_BASE "ut_misc_files/xml_files/same_subfolder_2/samename/samename.xml_pack@@file2_in_sub_2.xml");
