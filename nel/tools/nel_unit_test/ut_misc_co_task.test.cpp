...
TEST_F(CUTMiscCoTask, tasksAndThreads)
{
	...
	{
		ASSERT_EQ(referenceResultThread1[i], result2[i]);
	}
	for (uint i = 0; i < sizeofarray(referenceResultThread2); ++i)
	{
		ASSERT_EQ(referenceResultThread2[i], result[i]);
	}
}
TEST_F(CUTMiscCoTask, runThreads)
{
	...
	ASSERT_EQ(sizeofarray(referenceResult), result.size());
