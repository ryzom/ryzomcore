...
class CUTNetLayer3Test : public testing::Test
{
protected:
...
TEST_F(CUTNetLayer3Test, sendReceiveUpdate)
{
	...
	ASSERT_EQ(NbTestReceived, 2); // answer and reply
	...
	while (NbTestReceived < 20)
	{
		...
		ASSERT_TRUE((NbTestReceived == prevNbTestReceived) || (NbTestReceived == prevNbTestReceived + 1));
		NLMISC::nlSleep(10);
	}
	...
	while (NbTestReceived < 20)
	{
		...
		ASSERT_LE(NbTestReceived, prevNbTestReceived + 2);
	}
