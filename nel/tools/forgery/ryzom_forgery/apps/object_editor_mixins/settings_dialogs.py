...
rules, UI font/DPI, plus the panel's bottom bar (Save/Export/Quit). Live
data / repository paths / Ryzom tools folder settings live in
ryzom_paths_section.py instead (a standalone, cross-app component, not a
method here -- see its own module docstring). Split out of object_editor.py, see the "Split
					...
					for export_format in EXPORT_FORMATS:
						...
						if clicked:
							...
							self.export_dialog.quick_export(
								...
								self.search_paths_dialog.find_file, exports_dir,
				...
				for export_format in EXPORT_FORMATS:
					...
					if clicked:
						...
						self.export_dialog.export(
							self.shape_file.value, self._shape_source_name or "shape", export_format,
							self.search_paths_dialog.find_file, source_folder=source_folder,
							skeleton=self._bone_preview_skeleton, animation=self._bone_preview_animation)
				if workspace_dir is not None:
					imgui.separator()
					bnp_clicked, _ = imgui.selectable(f"Full workspace ({workspace_dir.name}.bnp)", False)
