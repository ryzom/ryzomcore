...
**Split en cours (voir `project-todos/ryzom-core/forgery-object-editor.md`) :**
...
grille/axes, transparence, wireframe, mode de shading, vent, panneau transform, framing caméra,
...
- Panneau flottant "Skinning preview" (`_draw_bone_preview_controls`, creature_bind.py) : choix squelette/animation (scannés via `search_paths_dialog`), mis en évidence en vert. Priorité aux vraies données réelles (`creature_full_index.creatures_for_shape()` — le shape chargé est retrouvé dans le vrai `.creature` qui l'équipe, via `settings.live_data_path`, voir `live_data.py`/`creature_full_index.py`/`live_data_index_dialog.py`) ; repli sur l'ancienne heuristique par comparaison de noms d'os (`SearchPathsDialog.compatible_for()`/`compatible_animations_for()`) uniquement pour un shape absent de cet index.
...
- Helpers de viewport (grille au sol, axes monde/pivot, transparence objet, wireframe, mode de shading) : `_rebuild_viewport_helpers` (object_editor.py), toggles associés (object_editor.py). Transparence (`_object_transparent`/`_apply_object_transparency`, `viewport_transform.py`) indépendante et combinable avec le wireframe et le shading -- `TransparencyAttrib`/`ColorScaleAttrib`, ré-appliquée après chaque `_rebuild_geometry` puisque `model_root` est détruit/recréé à chaque chargement/remplacement de shape. Dessinés par `_draw_viewport_toggles` (object_editor.py).
- **Wireframe à 3 états** (`self._wireframe_mode`, `"off"`/`"overlay"`/`"pure"`, `_apply_object_wireframe`/`_cycle_object_wireframe`/`_set_object_wireframe_mode`, `viewport_transform.py`, project-todos/forgery/`wireframe_cycle_states.md` -- remplace l'ancien booléen on/off) : `"off"` aucune surcharge ; `"overlay"` (ancien comportement) `set_render_mode_filled_wireframe((0, 0, 0, 1), 1)` -- arêtes noires par-dessus le rendu texturé/ombré normal, ne le remplace jamais (`object_editor__wireframe_overlay.md`) ; `"pure"` (nouveau) `set_render_mode_wireframe(1)` + `set_texture_off(1)` -- filaire seul, sans texture ni remplissage. Même bouton/icône dans les 3 états (`ICON_FA_DRAW_POLYGON`, actif dès que l'état n'est pas `"off"`), clic gauche cycle, clic droit ouvre un popup listant les 3 états pour y sauter directement -- même convention générale que le bouton de shading ci-dessous. Ré-appliqué après chaque `_rebuild_geometry` comme la transparence.
- **Bouton cyclique de mode de shading** (`self._shading_mode`, `"shading"`/`"constant"`/`"unshaded"`, `_apply_shading_mode`/`_cycle_shading_mode`/`_set_shading_mode`, `viewport_transform.py`) : `"shading"` (défaut) n'applique aucune surcharge, hérite normalement de l'ambiant + `self.sun_light_np` comme le reste de la scène ; `"constant"` coupe uniquement `self.sun_light_np` sur `model_root` (`set_light_off(light, priority)`, la forme à une seule lumière, pas la forme "toutes lumières") -- l'ambiant globale reste active, donnant un rendu plat sans ombre/relief directionnel mais toujours texturé ; `"unshaded"` coupe toute lumière (`set_light_off(1)`) et toute texture (`set_texture_off(1)`) et peint `self._unshaded_color` en couleur unie sur tout le modèle (`set_color(..., 1)`). Convention générale pour tout bouton à cycle dans Forgery : clic gauche = `_cycle_shading_mode()` (état suivant) ; clic droit = popup (`imgui.begin_popup_context_item`) listant les 3 états pour y sauter directement. En mode `"unshaded"`, une pastille de couleur (`imgui.color_edit3`, flag `no_inputs`) apparaît à côté du bouton pour choisir `self._unshaded_color` (non persisté, blanc par défaut). **Intensité ambiante au maximum en `"constant"`** : `_transition_shading_mode()` (appelée par `_cycle_shading_mode()`/`_set_shading_mode()`, jamais par `_apply_shading_mode()` elle-même -- qui ne fait que ré-appliquer l'état courant après `_rebuild_geometry`, sans re-déclencher la sauvegarde) sauvegarde `self._ambient_light_intensity` dans `self._saved_ambient_light_intensity` et la monte à `_LIGHT_INTENSITY_MAX` en entrant dans `"constant"`, la restaure en le quittant vers `"shading"` ou `"unshaded"`. Indépendant de la transparence/du wireframe, combinable avec les deux. Ré-appliqué après chaque `_rebuild_geometry` comme les deux autres.
- Couleur de fond de la viewport (`_draw_background_color_bar`, `viewport_transform.py`) : barre verticale flottante juste au-dessus de `_draw_viewport_toggles`, même position x/même mécanisme d'auto-mesure. 5 pastilles (`self._viewport_bg_colors`, object_editor.py) : noir, gris (`(0.41, 0.41, 0.41)`, couleur de fond par défaut de Panda3D), vert écran, et 2 emplacements libres. Clic gauche applique la couleur (`self.setBackgroundColor`, `_set_viewport_background_color`) ; clic droit sur un des 2 emplacements libres ouvre un color picker (`imgui.color_edit3`) pour le recolorer. Aucune couleur n'est forcée au démarrage — l'app garde la couleur de fond par défaut du moteur tant que l'utilisateur n'a pas cliqué une pastille.
- Panneau Position/Rotation/Scale (`_draw_transform_panel` et méthodes associées, object_editor.py) : verrouillage par axe et par "pivot" (édite `model_root` local au lieu de `_object_pivot`), reset par ligne ou global (bouton Home du nav cube via `reset_object_transform`, object_editor.py). Champs X/Y/Z en `imgui.drag_float` (`_TRANSFORM_DRAG_PARAMS`, object_editor.py) — glisser pour ajuster, double-clic/Ctrl+clic pour taper une valeur exacte. Ligne Scale uniquement : icône de lien à bascule (`self._scale_axes_linked`, object_editor.py) — actif, éditer un axe X/Y/Z impose sa valeur aux deux autres axes non verrouillés individuellement (`_set_transform_axis`, viewport_transform.py) ; activer le lien seul ne change rien tant qu'aucun axe n'est ensuite édité. État transitoire, jamais persisté (comme les verrous d'axe). Le Ctrl+drag du gizmo 3D lui-même (`camera.py::ObjectManipulator._scale()`) est déjà uniforme par nature, indépendamment de ce lien.
- **Verrou de pivot : bake dans les sommets, pas dans le pivot** (`_bake_locked_edit_into_vertices`, `_bake_transform_into_shape`, shape_io.py) : le fichier `.shape` ne stocke qu'une seule position/rotation/scale par shape (`base.default_pos`/`default_rot_quat`/`default_scale`), sans notion séparée de "pivot" — `_bake_transform_into_shape` ne lit donc que `_object_pivot`, jamais `model_root`, pour ces champs, si bien que le pivot n'est jamais modifié par une édition verrouillée. À la sauvegarde, l'offset local courant de `model_root` (l'édition verrouillée) est appliqué directement aux sommets du mesh (`geom.vertex_buffer.channels["Position"]`/`"Normal"]`, translation + rotation + scale autour de l'origine locale, `bbox` recalculée), puis `model_root` est remis à l'identité — uniquement pour un `CMesh` classique (`MRM`/`MRMSkinned`/`MultiLod` : géométrie non éditable par pynel, le bouton de verrou de pivot est grisé pour ces types). Le binding de skin (`Weight`/`PaletteSkin`) n'est pas affecté par ce bake, seule la position de repos des sommets change — un pivot verrouillé reste donc éditable sur un `CMesh` skinné, contrairement au reste du panneau (qui édite `base.default_*`, sans effet en jeu pour un skinné, `transform.cpp:946`).
...
- Section dépliable "Texture offset/tiling/rotation" (`_draw_material_texture_transform_section`, object_editor.py) : plus de case à cocher "activer" (retirée, jugée sans utilité réelle — les sliders sont toujours visibles et affectent directement `Material.flags` dès qu'une vraie valeur est modifiée). Édite `Material.tex_user_mat[0]` via `decompose_uv_matrix`/`compose_uv_matrix` (shape_geometry.py), qui traduisent la matrice générique NeL en (offset U/V, échelle U/V, rotation) en supposant l'absence de cisaillement — vrai pour tout usage réel connu (rollout UV Offset/Tiling/Angle de 3dsMax). Reflété dans le viewport 3D via `NodePath.set_tex_transform()` (`_apply_material_texture`, object_editor.py) et `shape_geometry.py`'s `uv_matrix_to_panda_mat4()` — Offset U/V, l'inversion de l'axe V et le sens de la Rotation sont confirmés corrects par test contre le vrai client. Scale U/V et Rotation sont mutuellement exclusifs dans l'UI (grisés tant que l'autre n'est pas à sa valeur neutre) : leur combinaison produit un artefact de répétition GPU (voir `docs/material_options.md`'s "Matrice de texture exportée"), et n'est utilisée par aucun shape réel du jeu (0/2600 shapes scannés dans `ryzom_live/data`).
- Section dépliable "Smoothing" (`_draw_material_smoothing_section`, materials.py) : case "Smooth normals" + slider d'angle 0-180° + bouton "Apply", lisent/écrivent `Material.smoothing_angle` (champ natif `.shape`, sentinelle `-1.0` = jamais réglé — voir `docs/material_options.md`'s "Lissage des normales"). Bouger le slider seul ne change rien à l'affichage : le recalcul (`smooth_normals_per_material`/`_bake_smoothed_normals_into_shape`, shape_geometry.py/shape_io.py) ne se déclenche qu'au clic Apply, à la sauvegarde, ou à l'ouverture suivante du fichier — toujours reparti d'un snapshot pristine de la géométrie capturé au chargement (`_display_shape`), jamais du buffer déjà baké, pour que remonter/redescendre l'angle plusieurs fois avant de sauvegarder reste réversible. `_apply_smoothed_normals`/le rafraîchissement post-Save utilisent `_rebuild_geometry_preserving_camera` (pas `_rebuild_geometry` directement) pour ne pas recentrer la caméra à chaque clic. Portée actuelle : `CMesh` uniquement (un seul `VertexBuffer` partagé par tous les `rdr_pass`) -- `CMeshMRM`/`CMeshMultiLod`/`CMeshMRMSkinned` (plusieurs LODs/geomorphs partageant un buffer) pas encore supportés.
...
- **Section "Ryzom Paths" (`ryzom_forgery/ryzom_paths_section.py`)** : Settings > Ryzom Paths, regroupe sous un header unique 3 réglages génériques de toute la suite Forgery (pas propres à Patina) -- réutilisable tel quel par n'importe quelle app (voir `docs/apps/landscape_editor.md`'s "Onglets" pour Atyscape) :
  - `RepositoryPathsDialog` (`ryzom_forgery/repository_paths_dialog.py`, extrait de `settings_dialogs.py`) : un sélecteur de dossier par dépôt (`pynel.repository_paths.REPOSITORIES` -- ryzom-core/ryzom-data/ryzom-private-data/ryzom-docker), même motif que `workspace_setup_dialog`/`_draw_workspace_sync_settings`. Bouton Clone (icône téléchargement) à côté de chaque dépôt — ouvre un sélecteur de dossier parent, puis clone via `pynel.repository_paths.clone_repo()`. Désactivé si le dépôt est déjà configuré et que le dossier existe (tooltip explicite). Persisté par pynel (`repository_paths.json`), partagé avec tout autre outil basé sur pynel — voir `docs/repository_paths.md` de pynel.
  - `LiveDataSetupDialog`/`LiveDataIndexDialog` (`live_data_setup_dialog.py`/`live_data_index_dialog.py`) : `settings.live_data_path` -- le dossier "data" d'une install Ryzom Live réelle (`creature.packed_sheets` etc), auto-détecté via `ryzom.ini` (`live_data.py`, popup bloquante seulement si la détection échoue -- popup Patina-only, câblée dans `draw_ui()`, pas dans `RyzomPathsSection`). Alimente `creature_full_index.py` (index réel shape → créature/skel/animset, sha1-invalidé, reconstruit par `LiveDataIndexDialog` en fond avec popup + barre de progression) utilisé par le panneau "Skinning preview" ci-dessus (Patina uniquement). Bouton refresh (icône 🔄) à côté du chemin pour forcer une reconstruction sans attendre un changement de sha1.
  - `RyzomToolsSetupDialog` (`ryzom_forgery/ryzom_tools_setup_dialog.py`) : `settings.ryzom_tools_path`, dossier contenant les exécutables natifs du pipeline Ryzom (`zone_welder`, etc.) -- consommé par Atyscape uniquement aujourd'hui, mais éditable dans les deux apps par cohérence avec les 2 réglages ci-dessus.

  Patina et Atyscape sont deux apps indépendantes à publics disjoints (graphistes vs level designers) : un utilisateur de l'une ne doit jamais avoir à ouvrir l'autre pour configurer un réglage dont lui seul a besoin -- d'où l'extraction de ces 3 réglages hors du mixin `settings_dialogs.py` propre à Patina, dans un composant partagé.
...
- **Rendu** (`_apply_loaded_shape_to_creature`, object_editor.py) : le shape chargé
  ...
  `model_root` est recréé visible par défaut à chaque rebuild (Save, bouton Apply du
  Smoothing, case à cocher Skinned de l'onglet Geometry, remplacement de géométrie via
  Import), donc `_apply_loaded_shape_to_creature` doit être rappelée à chaque fois pour
  le recacher tant qu'un PNJ est affiché.
...
**Mode/animation + lecture live.** Un combo "Mode" (NORMAL/COMBAT/
...
- `import_watcher`, `workspace_sync` et `tex_dds_sync` (conversion `tex/` → `dds/`, voir `docs/tex_dds_sync.md`) ne possèdent plus chacun leur propre `Observer` dédié -- ils partagent un unique `self.workspace_watch` (`ryzom_forgery.workspace_watch.WorkspaceWatcher`, voir `docs/workspace_watch.md`), sur lequel chacun enregistre sa callback `handle_settled` pour le(s) sous-dossier(s) qui le concernent (`"imports"`, `"tex"`, et `workspace_sync.SYNCED_SUBDIRS` = `anims`/`shapes`/`skels`/`dds`). Les `Observer` séparés n'apportaient aucun bénéfice réel (voir `docs/workspace_watch.md`), c'était de la duplication organique.
- Réglages Settings → Paths/Tools (`object_editor_mixins/settings_dialogs.py`) : `_draw_workspace_sync_settings` (bouton "Sync now" si pas totalement synchronisé), `_draw_image_editor_settings`, `_draw_text_editor_settings` (même motif — utilisé par le bouton crayon du `panoply.cfg`, voir "Intégration Panoply"), `_draw_ui_font_settings` (nécessite un redémarrage manuel de Patina -- pas de bouton "Restart now" dans l'app, retiré : ne fonctionnait pas lancé via ryztart).
...
| `ryzom_forgery.workspaces` (`SUBDIRS`, `ensure_structure`, `reveal_in_system_file_manager`) | Structure de dossiers du workspace (dont `dds/`) ; révélation d'un fichier dans le gestionnaire système (vignettes de texture). |
