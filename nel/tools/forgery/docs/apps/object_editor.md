...
- `_bake_panoply_real` (object_editor.py) : bake **réel**, port exact `panoply_maker.cpp` (`panoply_bake.py`, voir `docs/panoply_bake.md`) — distinct de l'approximation live ci-dessus. Vérifie d'abord `pynel.repository_paths.is_valid("ryzom-data")` : si `ryzom-data` n'est pas configuré (2026-08-29), saute vers Settings > Paths et fait clignoter le champ (`self.request_settings_attention("Paths", "ryzom-data")`, voir "Settings-attention" ci-dessous) et **ne fait rien d'autre** (aucun fichier écrit) — remplace l'ancienne popup modale bloquante. Sinon, résout la race depuis le nom de la texture (`panoply_config.RACE_PREFIX_TO_TABLE`), construit les axes candidats (`panoply_bake.axes_for_source`), résout chaque masque via `search_paths_dialog.find_texture` (uniquement des fichiers réels sur disque, `FoundEntry.fs_path` — un fichier vivant seulement dans un `.bnp` ne peut pas être baked depuis ici), puis écrit chaque variante dans `tex/` et le `.hlsinfo`/`panoply_files.txt`/`characters.hlsbank` mis à jour dans `build/` du workspace actif via `panoply_bake.bake_and_write` (sources de départ : `characters.hlsbank`/`panoply_files.txt` réels sous `<ryzom-data>/final_bnps/characters_maps_hr/`, jamais écrasés — voir `docs/panoply_bake.md`).
- `_bake_panoply_real_all` (object_editor.py, 2026-08-29) : boucle sur `_shape_texture_names()`, bake chaque texture qui a au moins un masque résolu (même vérification que `_draw_panoply_masks_for`) — bouton feu à côté du gear/crayon dans `_draw_global_panoply_section`, complète le bouton feu par-texture (qui reste disponible pour bake une seule texture).
