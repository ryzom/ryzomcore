...
- `_draw_repository_paths_settings` / `_poll_repository_paths_dialog` (`object_editor_mixins/settings_dialogs.py`, voir la note "Split object_editor.py into theme files" plus bas) : Settings > Paths, un sélecteur de dossier par dépôt (`pynel.repository_paths.REPOSITORIES` -- ryzom-core/ryzom-data/ryzom-private-data/ryzom-docker), même motif que `workspace_setup_dialog`/`_draw_workspace_sync_settings`. **Nouveau 2026-09-07** : bouton Clone (icône téléchargement) à côté de chaque dépôt — ouvre un sélecteur de dossier parent, puis clone via `pynel.repository_paths.clone_repo()` (`_poll_clone_target_dialog`, `_draw_clone_status`). Le bouton Clone est désactivé si le dépôt est déjà configuré et que le dossier existe (tooltip explicite). Persisté par pynel (`repository_paths.json`), partagé avec tout autre outil basé sur pynel — voir `docs/repository_paths.md` de pynel.
- `LiveDataSetupDialog`/`LiveDataIndexDialog` (`live_data_setup_dialog.py`/`live_data_index_dialog.py`, 2026-09-03, Patina-only -- pas un champ générique `ForgeryApp` comme `workspace_setup_dialog`) : Settings > Paths, `settings.live_data_path` -- le dossier "data" d'une install Ryzom Live réelle (`creature.packed_sheets` etc), auto-détecté via `ryzom.ini` (`live_data.py`, popup bloquante seulement si la détection échoue). Alimente `creature_full_index.py` (index réel shape → créature/skel/animset, sha1-invalidé, reconstruit par `LiveDataIndexDialog` en fond avec popup + barre de progression) utilisé par le panneau "Skinning preview" ci-dessus. Bouton refresh (icône 🔄) à côté du chemin pour forcer une reconstruction sans attendre un changement de sha1.
