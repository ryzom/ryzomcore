...
### Intégration Panoply (lecture/rendu uniquement)

- `_draw_global_panoply_section` (object_editor.py) : sélection globale par axe (skin/user/hair/eyes), affichée en haut des onglets Textures et Materials. Purement un override de rendu — ne modifie jamais les données du shape (`texture.file_name` reste inchangé). Règle particulière : le premier clic sur une sélection vide pré-remplit aussi les autres axes disponibles avec leur première valeur, car aucun fichier réel n'existe pour un seul axe isolé (ex. `..._FY.tga` n'existe pas, seul `..._FY_U1.tga` existe).
...
- Masques Panoply : `_draw_panoply_masks_for` (object_editor.py) affiche les vignettes des masques déjà présents pour un texture donné, et `_create_panoply_mask` (object_editor.py) permet de créer un masque noir (poids nul) vide directement dans `masks/` du workspace actif pour un axe manquant.
- `_update_texture_freshness` (object_editor.py) revérifie chaque seconde les combinaisons Panoply en jeu (`_panoply_texture_sources`/`_panoply_texture_signatures`) et les textures simples du workspace, évince du cache et force un réaffichage complet si quelque chose a changé sur disque — c'est la seule mécanique de "reload" de Patina, il n'y a pas de bouton "Reload" manuel (voir `.todo/forgery-object-editor.md`, Phase A Step 5, citée dans le commentaire).
...
- Réglages Settings → Paths/Tools : `_draw_workspace_sync_settings` (object_editor.py, bouton "Sync now" si pas totalement synchronisé), `_draw_image_editor_settings` (object_editor.py), `_draw_ui_font_settings` (object_editor.py, nécessite un redémarrage — `self.relaunch`).
...
| `ryzom_forgery.panoply`, `panoply_colorize`, `panoply_config`, `panoply_live`, `panoply_texture` | Pipeline Panoply complet, réutilisé en lecture/rendu (voir section dédiée ci-dessus). |
