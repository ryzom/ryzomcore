...
- `_mesh_global_matrix_ids(geom)` (`shape_geometry.py`, ajouté 2026-09-05) — pour un `CMesh` skinné : résout le canal `VertexBuffer["PaletteSkin"]` (indices **locaux au `MatrixBlock`** qui rend chaque sommet, voir `docs/shape_import.md`) en indices globaux (dans `geom.bones_name`), via `matrix_block.matrix_id`. Chaque sommet n'appartient qu'à un seul `MatrixBlock`, donc pas d'ambiguïté à résoudre en un seul passage.
- `_passes_from_mesh_geom(geom, skeleton=None, bone_world_matrices=None)` (`shape_geometry.py`) — cas `CMesh` simple : si `geom.skinned` et un squelette/pose sont fournis, skin les positions/normales via `_numpy_skin_batch` (avec `_mesh_global_matrix_ids` pour les indices d'os, `channels["Weight"]` déjà normalisé) avant d'itérer tous les `matrix_blocks` × leurs `rdr_passes` (aucune notion de LOD/geomorph, contrairement à `CMeshMRM`) ; sinon, rendu rigide inchangé.
...
- `load_panda_cube_texture(sub_textures, cache=None, search_dirs=None, repeat=False, finder=None)` (`shape_geometry.py`) — assemble une cube map Panda3D à partir des 6 faces d'une `CTextureCube` (chaque face chargée via `load_panda_texture`, même `cache` donc pas de re-décodage). L'ordre NeL (`texture_cube.h`'s `TFace` : positive_x/negative_x/positive_y/negative_y/positive_z/negative_z) correspond directement à l'ordre attendu par Panda3D pour les z-slices d'une cube map, aucun réordonnancement nécessaire. Une face manquante/non décodable retombe sur la première face décodée avec succès. Utilisé par `object_editor.py` pour le mode `CMaterial::TShader::Specular` (stage 1 = carte spéculaire/cubemap de réflexion).
...
- `apps/object_editor.py` importe `finest_skinned_lod, iter_render_passes, load_panda_cube_texture, load_panda_texture, resolve_texture_ref, rgba_to_color, shape_bbox` — l'éditeur 3D live s'appuie massivement sur ce module pour construire sa géométrie de rendu (`object_editor.py, 2053`) et gérer le rendu skinné avec fallback rigide (`object_editor.py`).
...
- `iter_render_passes` pour `MeshMRMSkinned` sans `skeleton`/`bone_world_matrices` ne lève pas d'erreur : il retombe silencieusement sur le rendu rigide bind-pose (`shape_geometry.py, 170-174`) — comportement voulu pour permettre d'inspecter un shape avant d'avoir chargé un `.skel` compatible. Même comportement pour `CMesh` skinné depuis 2026-09-05.
- Le panneau "Skinning preview" de Patina (`apps/object_editor_mixins/geometry_helpers.py::_is_shape_skinned`) ne reconnaissait, jusqu'au 2026-09-05, que `MeshMRM`/`MeshMRMSkinned` -- un `CMesh` skinné (produit par `shape_import.py` à l'import d'un `.dae`/`.fbx`/`.gltf`, voir `[[skinned_mesh_import]]`) ne s'activait jamais, silencieusement (bouton grisé, aucun message). Corrigé avec `[[mesh_skinning_preview]]` (`project-todos/forgery/`) : `_mesh_global_matrix_ids` ci-dessus, plus un triplet `_MeshSkinState`/`_build_mesh_skin_state`/`_reskin_mesh_state` dans `apps/object_editor_mixins/skin_state_helpers.py` (même principe que `_MrmSkinState` pour `MeshMRM`, sans étape de LOD/geomorph).
