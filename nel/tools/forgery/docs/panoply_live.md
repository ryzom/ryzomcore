...
- `LiveColorizeCache` — `panoply_live.py` : classe de mémoïsation.
  ...
  - `get(key)` / `set(key, image)` (`panoply_live.py`) : accès simple dict, aucune éviction.
...
- Aucune éviction de cache : le commentaire du docstring (`panoply_live.py`) justifie ce choix par le fait que le nombre de combinaisons distinctes réellement vues en session reste petit en pratique (quelques choix d'axes sur les textures d'un shape) — c'est un choix de design assumé, pas un oubli.
