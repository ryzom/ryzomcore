...
navigateur permanent ("Wexplorer", les dossiers épinglés) qui reste
accessible quel que soit l'endroit courant de l'arborescence principale
(`explorer.py`).
...
- `Explorer.__init__` (`explorer.py`) : construit l'état — racine
 ...
 callbacks (`on_selection_changed`, `extra_header`, `extra_toolbar`) et
 `pinned_folders`.
...
- **Pas de vignette pour un fichier à l'intérieur d'un `.bnp`** : `item.path`
 ...
 (`explorer.py`).
- **Filtre + recherche n'affectent pas le Wexplorer** : `_draw_tree_children`
 affiche tout, sans tenir compte de `self.search`/`self.extension_filter`,
 volontairement — c'est un panneau "tout parcourir", distinct de la vue
 principale filtrable (`explorer.py`).
