...
public:
	 ...
	 */
	virtual void		init(uint maxTrack = 32, bool useEax = true, bool useADPCM = true, NLMISC::IProgressCallback *progressCallBack = nullptr, bool autoLoadSample = false, TDriver driverType = DriverAuto, bool forceSoftware = false, bool manualRolloff = true) NL_OVERRIDE;
	...
	virtual void		initDevice(const std::string &deviceName, const CInitInfo &initInfo, NLMISC::IProgressCallback *progressCallback = nullptr) NL_OVERRIDE;
...
//	virtual void				loadEnvEffects( const char *filename );
	 ...
	 */
	virtual uint32				loadSampleBank(bool async, const std::string &name, std::vector<std::string> *notfoundfiles = nullptr) NL_OVERRIDE;
	/** Unload buffers.
	*/
...
//	virtual	void				loadEnvSounds( const char *filename, UEnvSound **treeRoot=NULL );
	 ...
	 * source.
	 */
	virtual USource				*createSource( const NLMISC::TStringId &name, bool spawn=false, TSpawnEndCallback cb = nullptr, void *cbUserParam = nullptr, NL3D::CCluster *cluster = nullptr, CSoundContext *context = nullptr, UGroupController *groupController = nullptr) NL_OVERRIDE;
	/// Add a logical sound source (by sound id). To remove a source, just delete it. See createSource(const char*)
	virtual USource				*createSource( TSoundId id, bool spawn=false, TSpawnEndCallback cb = nullptr, void *cbUserParam = nullptr, NL3D::CCluster *cluster = nullptr, CSoundContext *context = nullptr, UGroupController *groupController = nullptr) NL_OVERRIDE;
